...
class Cube:
    ...
    @staticmethod
    def get_dict_key(x, y, z):
        return f"{x}.{y}.{z}"

    @property
    def dict_key(self):
        return Cube.get_dict_key(self.x, self.y, self.z)

class Cubes:
    def __init__(self, cubes, xy_size, z_size=1, cycle=0):
        self.cubes = cubes
    ...
    def get_cube_neigbors(self, x, y, z):
        return [Cube.get_dict_key(x+p[0], y+p[1], z+p[2]) for p in itertools.product([-1,0,1], repeat=3) if p.count(0) != 3]
    ...
    def run_cycle(self, print_cubes=False):
        ...
        print(f"-- Starting cycle {self.cycle}, {self.z_size=}, {self.xy_size=} --\n")

        for z in self.get_z_range():
            for y in self.get_xy_range():
                for x in self.get_xy_range():
                    cube_key = Cube.get_dict_key(x,y,z)
                    cube_neighbors = self.get_cube_neigbors(x, y, z)
                    if cube_key in self.cubes:
                        cube_state = self.cubes[cube_key].state
                    else:
                        cube_state = CUBE_INACTIVE
                        next_cubes[cube_key] = Cube(cube_state, x, y, z)
